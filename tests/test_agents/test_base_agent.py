...
def test_agent_creation(bureau_skill, pillar_yaml, mock_llm, logger):
    agent = BaseSpecialistAgent(bureau_skill, pillar_yaml, mock_llm, logger)
    assert agent.skill.name == "bureau"
    assert agent.rolling_summary == ""


def test_build_system_prompt(bureau_skill, pillar_yaml, mock_llm, logger):
    agent = BaseSpecialistAgent(bureau_skill, pillar_yaml, mock_llm, logger)
...
def test_build_system_prompt_includes_rolling_summary(
    bureau_skill, pillar_yaml, mock_llm, logger
):
    agent = BaseSpecialistAgent(bureau_skill, pillar_yaml, mock_llm, logger)
...
def test_update_rolling_summary(bureau_skill, pillar_yaml, mock_llm, logger):
    agent = BaseSpecialistAgent(bureau_skill, pillar_yaml, mock_llm, logger)
...
def test_rolling_summary_truncation(bureau_skill, pillar_yaml, mock_llm, logger):
    agent = BaseSpecialistAgent(bureau_skill, pillar_yaml, mock_llm, logger)
    for i in range(50):
...
async def test_run_returns_specialist_output(bureau_skill, pillar_yaml, mock_llm, logger):
    ...
    agent = BaseSpecialistAgent(bureau_skill, pillar_yaml, mock_llm, logger)
    output = await agent.run("What is the credit risk?", mode="chat")
