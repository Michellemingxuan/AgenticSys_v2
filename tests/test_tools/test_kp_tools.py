"""Tests for tools/kp_tools.py — _active_kps and _format_kp_digest."""
...
def test_format_kp_digest_empty_when_no_kps():
    assert _format_kp_digest([]) == ""
    assert _format_kp_digest(None) == ""


def test_format_kp_digest_renders_active_set_only():
    """The digest lists active topic names and points to kp_lookup tools."""
    kps = [
    ...
    digest = _format_kp_digest(kps)
    assert "fico_trajectory" in digest    # topic name shown
    assert "kp_lookup" in digest          # points to lookup tool
    assert "(1)" in digest                # deduped to 1 active topic


def test_format_kp_digest_shows_cross_specialist_topics():
    """When full_kps is provided, digest includes other specialists' topics."""
    ...
    digest = _format_kp_digest(own_kps, full_kps=full_kps, self_name="strategy")
