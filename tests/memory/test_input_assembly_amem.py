...
episodic window) + episodic (recent turns) + KP-warmth (topics) + question."""
...
def _sess():
    return SimpleNamespace(
        specialist_kps={"risk": [{"topic": "tsr", "claim": "x" * 500}]},
        qa_cache={},
...
def test_case_summary_block_injected_alongside_warmth():
    ...
    assert "KP-warmth" in out                 # warmth is always built now
