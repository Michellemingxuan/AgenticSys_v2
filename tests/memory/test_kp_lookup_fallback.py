"""kp_lookup is a RAM-only cache: a slug hit returns the cached KP; a miss tells
...
def test_kp_lookup_hits_ram_cache():
    app_ctx = SimpleNamespace(_specialist_kps={
        "risk": [{"topic": "tsr_peak", "claim": "TSR peaked at 39.6",
                  "numbers": [{"period": "2024-09", "value": 39.6}]}]})
    ctx = SimpleNamespace(context=app_ctx)
    out = asyncio.run(kps.kp_lookup.on_invoke_tool(ctx, '{"topic": "tsr_peak"}'))
    assert "TSR peaked at 39.6" in out
    assert "39.6" in out


def test_kp_lookup_miss_is_ram_only_no_amem_call():
    ...
    app_ctx = SimpleNamespace(_specialist_kps={}, _amem=fake, _amem_cfg=CFG, _case_id="c1")
    ctx = SimpleNamespace(context=app_ctx)
    out = asyncio.run(kps.kp_lookup.on_invoke_tool(ctx, '{"topic": "tsr_peak"}'))
