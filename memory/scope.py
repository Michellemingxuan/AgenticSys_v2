...
def kps_for_turn(specialist_kps: dict, turn_id: str) -> list[str]:
    out: list[str] = []
    for kps in (specialist_kps or {}).values():
        for kp in kps or []:
...
def kps_for_agent_turn(specialist_kps: dict, agent_id: str, turn_id: str) -> list[dict]:
    ...
    out: list[dict] = []
    for kp in (specialist_kps or {}).get(agent_id, []) or []:
        if kp.get("captured_at_turn") == turn_id:
            out.append(kp)
    return out
