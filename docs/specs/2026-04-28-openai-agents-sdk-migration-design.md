...
2. **SDK API for partial trace state on `Runner.run` exception** — exact attribute name (`exc.run_result.new_items`?) and item type (`ToolCallOutputItem`?). Drives the §5.3 fallback implementation.
   ...
   **§7-2 findings (resolved 2026-04-28):**
   - Exception type for retry/turn exhaustion: `MaxTurnsExceeded` (subclass of `AgentsException`; module `agents.exceptions`). All `AgentsException` subclasses (including `ModelBehaviorError`, `UserError`, and the guardrail tripwire classes) carry a `run_data` attribute populated by the runner before re-raising.
   - Partial trace access: `exc.run_data.new_items` — where `exc.run_data` is a `RunErrorDetails` dataclass (fields: `input`, `new_items`, `raw_responses`, `last_agent`, `context_wrapper`, `input_guardrail_results`, `output_guardrail_results`). The runner's `except AgentsException` block sets `exc.run_data` before re-raising, so it is populated for *any* `AgentsException` including `MaxTurnsExceeded`. Note: the spec's original guess of `exc.run_result.new_items` was wrong — it is `exc.run_data.new_items`.
   - `ToolCallOutputItem` class: confirmed as `agents.items.ToolCallOutputItem` (dataclass with fields `agent`, `raw_item`, `output`, `type`). The completed tool-call payload is accessed via `.output` (the Python object returned by the tool function) and `.raw_item` (a `FunctionCallOutput` TypedDict with keys `call_id`, `output` [JSON string], `type`, `id`, `status`). For recovering serialised Pydantic objects use `ToolCallOutputItem.output` directly (already the Python return value), or `ToolCallOutputItem.raw_item["output"]` for the JSON string.
   - Successful `RunResult` structure (for reference): fields `input`, `new_items`, `raw_responses`, `final_output`, `input_guardrail_results`, `output_guardrail_results`, `tool_input_guardrail_results`, `tool_output_guardrail_results`, `context_wrapper`, `_last_agent`. The `new_items` field on `RunResult` and `RunErrorDetails` are both `list[RunItem]`, making the fallback code symmetrical with the happy path.
   - SDK version verified: `openai-agents 0.3.3`. Live run was not executed (no `OPENAI_API_KEY` available in CI env); all findings are from static source inspection of the installed package. The `exc.run_data` assignment in `agents/run.py` is unambiguous and not gated on any version flag.
