...
- [`.claude/memory/safechain_async_and_thread_occupation.md`](memory/safechain_async_and_thread_occupation.md) — "stuck at team construction" / "input not captured" = hung safechain calls leaking threads. Correct model-calling: build via `await amodel(model_id)` (`from safechain.core.model import amodel`; cache + rebuild on 401), then sync `chain.invoke` (`prompt | model | StrOutputParser()`) in a dedicated thread pool bounded by a per-call `asyncio.wait_for`. Do NOT use `chain.ainvoke`. Don't restore f15d6b9's reverted thread pool.
