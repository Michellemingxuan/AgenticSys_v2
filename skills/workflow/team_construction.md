...
|---|---|---|
| **spending / spend pattern / merchant concentration** | `spend_payments` + `modeling` (+ `crossbu` only if explicitly B2B) | `spend_payments` = transaction-level: monthly spend trend, top merchants (by spend + frequency), industry concentration, spend-to-payment ratio. Frame sub-question as "full spending pattern analysis" so the specialist covers temporal shape AND merchant/industry concentration (see its multi-aspect menu). `modeling` = ML-derived spend features only (out-of-pattern, concentration risk-rate, spend divergence — NOT output scores like TSR/CDSS). A spending answer with only `spend_payments` is incomplete — it misses the model's view of the spend shape. |
