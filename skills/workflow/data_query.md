...
- **"recent spike" / "crossed the threshold" → read `summary.threshold`, NOT `summary.peak`.** `peak` is the GLOBAL maximum and is usually an OLD month; it will point you at the wrong period. `summary.threshold` gives the catalog limit, `breaching_periods` (most recent last) and `latest_breach` — the recent crossing IS `latest_breach`. A score can breach in a recent month while `slope` is negative and `peak` sits a year earlier; that is a real recent spike, not a contradiction. Quote the breach period AND its value against the limit ("TSR 27.4 in 2025-04, above the 20 threshold").
