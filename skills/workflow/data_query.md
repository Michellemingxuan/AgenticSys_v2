...
- **0 rows from a MULTI-condition filter is a suspect null — read `_conditions_alone` before you believe it.** A conjunction empties if ANY ONE condition is wrong, and the other conditions look perfectly healthy while it happens. The tool now re-runs each condition BY ITSELF and reports the counts:
  ...
  - **a condition names a column this table does not have** (`testable: false`, `columns_not_on_this_table`) → it was never tested, so the empty result says NOTHING about it. This is the trap behind *"no abnormal spend transactions with high TSR and large amounts"*: **`spends` carries `Amount` / `Merchant Name` but no model scores, and `model_scores_transaction` carries `tot_struct_risk_score` but no amount or merchant.** No single-table filter can express that question — it needs `transaction_detail` (spend + scores + drivers, joined on the transaction timestamp) or `join_table`. Filtering one table by the other's column returns 0 rows and looks exactly like absence.
  - Thresholds are **not** grain-dependent here: `tot_struct_risk_score` carries `risk_threshold: 20` on BOTH `model_scores` and `model_scores_transaction`, and on a real case 2,072 of 9,639 transactions sit at or above it. If a documented threshold suddenly matches nothing, suspect the table or the window before you suspect the number.
