...
**Use the KP cache for cross-domain context.** When your question references a score reaction (e.g. "actions taken when TSR was reacting"), call `kp_list_topics()` or `kp_lookup(topic)` first — the modeling or bureau specialist likely already trended that score. Use the cached data to anchor your strategy-action timeline to the score movement. Do NOT re-run `summarize_trend` on `model_scores` or `bureau` columns if the KP cache already has that data.
