...
def _compose_orchestrator_instructions(
    ...
    parts = [
        ...
        (
            ...
            "This holds for EVERY question type — INCLUDING pure extraction / "
            "list pulls (\"extract the transactions where …\"): report_agent "
            "returns 'not covered by the reports' when nothing is relevant, "
            "which is an EXPECTED, REQUIRED outcome — never a reason to skip it. "
            "\n\nONE EXCEPTION, and it is NARROW: when the curated report is "
            "itself the SUBJECT of the question — the reviewer is asking what "
            "the report SAYS, not what the data shows — `report_agent` alone is "
            "the complete answer and you may skip the domain specialists. "
            "Qualifying: \"summarize the spending patterns FROM THE REPORT\", "
            "\"what does the report say about DPD\", \"per the curated reports, "
            "what were the main risks\". NOT qualifying, dispatch normally: "
            "\"what is the spending pattern\", \"any large spends\", \"is there "
            "a default pattern\" — a reviewer asking about the CASE gets live "
            "data even when the reports happen to discuss the same topic. "
            "The test is whether removing the reports would make the question "
            "unanswerable (exception applies) or merely less contextualised "
            "(dispatch specialists). When in DOUBT, dispatch — a wasted "
            "specialist round costs seconds; an answer that quietly substitutes "
            "report narrative for live data is the failure this whole protocol "
            "exists to prevent. Under the exception, attribute plainly in the "
            "answer (\"the curated reports state …\") so the reviewer knows the "
            "source was the report and not a fresh data pull. "
